...
The add-on source is owned by this repository at `xenforo-addon/`. The copy under
`public_html/src/addons/WindowsBuilds/` is deployment output. Export it with
`npm run deploy:addon`; `npm run build:full` performs that export automatically.
Do not edit the deployed copy directly or add a nested Git repository there.
